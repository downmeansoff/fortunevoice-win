...
[![Tests](https://img.shields.io/badge/tests-588-brightgreen.svg)](tests/)
...
The tray icon is the app mark, tinted: grey while the model loads, pale idle, red
...
| `FVOverlapDictation` | `true` | Start the next dictation while the previous one is still being decoded and typed. The recorder is free by then (the decode works on a copy of the samples), so waiting only cost a second or two of standing still between sentences. |
...
588 tests, run on every push by CI: the macOS project's own suite ported over (sentence splitting,
...
part of the default run. 621 tests in total: 588 by default, 21 UI, 8 live
and 4 end-to-end.
