...
def test_retype_is_offered(tray):
    ...
    The label is asked for by key rather than spelled out: the runner's system
    language decides whether the menu comes out Russian or English, and the
    item has to be offered either way.
    """
    from fortunevoice.strings import t

    tray.app.last_transcript = "проверка"
    labels = [str(i.text) for i in walk(tray._menu())]
    assert t("tray.retype_last") in labels, labels
